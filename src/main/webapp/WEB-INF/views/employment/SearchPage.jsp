			...
			<div class="row">
				...
				<div class="card mt-1 mb-4 shadow rounded-3">
					<div class="table-responsive">
							...
							<ul class="nav nav-pills" role="tablist">
								<li class="nav-item" role="presentation">
									<button class="nav-link active" data-bs-toggle="pill"
										role="tab" aria-selected="true" onclick="changeVal1()">채용정보</button>
								</li>
								<li class="nav-item" role="presentation">
									<button class="nav-link" data-bs-toggle="pill" role="tab"
										aria-selected="false" onclick="changeVal2()">기업정보</button>
								</li>
							</ul>
						</nav>

						<table class="table table-hover" id="section1"
							...
							<tbody>
								<c:forEach items="${epList }" var="employ">
									<tr>
										<td><a
											href="${pageContext.request.contextPath }/viewEpInfo?epnum=${employ.epnum }">
												${employ.epciname }</a></td>
										<td>${employ.epname }<input type="button" class="scrap"
											id="${employ.epnum }"
											onclick="checkVal('${employ.epnum }', this)" value="⭐">
										</td>
										<td>${employ.epdeadline }</td>
										<td>
											<button class="btn btn-secondary mt-1"
												onclick="WriteResume()"
												style="min-width: 75px; font-size: 20%;">즉시지원</button>
										</td>
									</tr>
								</c:forEach>
						...
						<table class="table table-hover d_none" id="section2"
							style="border-radius: 50px; text-align: center;">
							<thead>
								<tr>
									<th scope="col">기업명</th>
